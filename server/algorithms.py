...
def bubble_float_algo(nodes,
    ...
    node_data = zip(nodes, node_weights, node_parents, node_depths)
    layer_list = [[node] for node in node_data]
    # sort by weight and then by reverse depth
    layer_list.sort(key= lambda x: (x[0][1], -x[0][3]), reverse=True)
    layer_list = [deque(layer) for layer in layer_list]
...
# TODO: REMOVE BELOW
def bubble_float_algo_old(nodes,
                      node_weights,
                      node_parents,
                      gap_break = 3,
                      block_merge = False):
    ...
    #     parents : list of list of parent nodes for each node
    node_parents = [set(parents) for parents in node_parents]
    node_data = zip(nodes, node_weights, node_parents)
    layer_list = [[node] for node in node_data]
    layer_list.sort(key= lambda x: x[0][1], reverse=True) # sort by weight
    layer_list = [deque(layer) for layer in layer_list]
    # curr_layer = len(layer_list) - 1
    # print(layer_list)
    curr_l = len(layer_list) - 1
