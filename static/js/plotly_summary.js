...
function prepare_boxplot_data(plot_dat, style="Wiskers and Outliers") {
  ...
  for (let i = 0; i < plot_dat.names.length; i++) {
    ...
    trace.name = plot_dat.names[i];
    all_traces.push(trace);
  }
  return all_traces;
}

let in_data = {"names": ["All Points","Only Wiskers","Suspected Outlier",'Wiskers and Outliers'],
               "values":[[0.75, 5.25, 5.5, 6, 6.2, 6.6, 6.80, 7.0, 7.2, 7.5, 7.5, 7.75, 8.15, 8.15, 8.65, 8.93, 9.2, 9.5, 10, 10.25, 11.5, 12, 16, 20.90, 22.3, 23.25],
[0.75, 5.25, 5.5, 6, 6.2, 6.6, 6.80, 7.0, 7.2, 7.5, 7.5, 7.75, 8.15, 8.15, 8.65, 8.93, 9.2, 9.5, 10, 10.25, 11.5, 12, 16, 20.90, 22.3, 23.25],
[0.75, 5.25, 5.5, 6, 6.2, 6.6, 6.80, 7.0, 7.2, 7.5, 7.5, 7.75, 8.15, 8.15, 8.65, 8.93, 9.2, 9.5, 10, 10.25, 11.5, 12, 16, 20.90, 22.3, 23.25],
[0.75, 5.25, 5.5, 6, 6.2, 6.6, 6.80, 7.0, 7.2, 7.5, 7.5, 7.75, 8.15, 8.15, 8.65, 8.93, 9.2, 9.5, 10, 10.25, 11.5, 12, 16, 20.90, 22.3, 23.25],
]};

let summary_data = {"empirical_power": in_data,
                    "empirical_fdr": in_data,
                    "num_rejections": in_data,
                    };

$(".plotly-div").css("width", config.full_mirror.svg.width / 3);
$(".plotly-div").css("height", config.full_mirror.svg.width / 2);
// $(".plotly-box-divs").css("height", 200);
...
plotly_boxplot(config, "update", summary_data);


// function update_bar(data, graph_plotly_config){
...
//   Plotly.redraw(graph_div);
// }
// // Plotly.newPlot('ploty_all_summary', data, layout);
...
//         for(let i = 0; i < bar_names.length; i++){
//           max_val[ind] = Math.max(max_val[ind],
//                             data[signal_i][metric_i][method_i]["mean"][i] +
//                             data[signal_i][metric_i][method_i]["err"][i] )
//         }
//       }
//       ind = ind + 1;
//     }
